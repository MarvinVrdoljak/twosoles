...
// then the "needs", numbered "steps", the three views (each with its own QR
// code) and the tips. jsPDF + qrcode are imported lazily and the app's fonts
// are embedded, mirroring eventPdf.ts.
...
export async function downloadGuidePdf(content: GuidePdfContent) {
  ...
  content.needs.forEach((need) => {
    const lines = wrap(need, 10.5, contentW - 7)
  ...
  // ---- The flow (numbered steps) ----------------------------------------

  sectionHeading(content.stepsTitle)
  ...
  // ---- The 3 views (each with a QR code) --------------------------------
  ...
  content.views.forEach((view, index) => {
    ...
    urlLines.forEach((line, i) =>
      doc.text(line, textX, y + 5 + lines.length * 5.2 + i * 4),
    )
  ...
  content.tips.forEach((tip) => {
    const lines = wrap(tip, 10.5, contentW - 7)
    ensure(lines.length * 5.3 + 2)
    doc.setFillColor(...PRIMARY)
    doc.circle(marginX + 1.1, y - 1.3, 0.9, 'F')
    doc.setFont(body, 'normal')
